...
class Commander(Node):
    ...
    def timer_callback(self):
        ...
        # opposite phase
        if(mode_selection == 1):
            #self.get_logger().info('Mode: Opposite Phase')
...
class CmdVelSubscriber(Node):
    ...
    def listener_callback(self, data):
        ...
        self.get_logger().info(f'Received cmd_vel: linear.x={data.linear.x}, linear.y={data.linear.y}, angular.z={data.angular.z}')

        if abs(data.angular.z) > 0.5:
            mode_selection = 3
        elif abs(data.linear.y) > 0.5:
            mode_selection = 2
        elif abs(data.linear.x) > 0.5:
            mode_selection = 1
        else:
            mode_selection = 4        
