...
class UGVController(Node):
    ...
    def __init__(self):
        ...
        self.cable_length = 0.8    
        self.safety_margin = 0.3   
        self.tether_coef = 1.0
        self.effective_radius = 0.023

        self.kp_winch = 200.0
        self.ki_winch = 15
        self.kd_winch = 1.5
        self.velocity_winch_limit = 10.0
    ...
    def calculate_winch_velocity(self):
        ...
        dead_zone = 5.0
        if abs(winch_velocity) < dead_zone:
            winch_velocity = 0.0

        self.cable_length += winch_velocity * dt * self.effective_radius
        self.previous_error = length_error 
        if winch_velocity < 0.0:
            self.effective_radius = 0.023 + self.distance/1000     
        else:
            self.effective_radius = 0.023 - self.distance/1000
        # self.get_logger().info(f'effective_radius: {self.effective_radius:.5f}, Winch velocity {winch_velocity:.5f}, Error: {length_error:.5f}')
    ...
    def control_loop(self):
        ...
        cable_length_msg.data = [float(self.cable_length), float(self.target_length), float(self.distance)]
