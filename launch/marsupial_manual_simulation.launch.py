...
def generate_launch_description():
    ...
    world_file_name = LaunchConfiguration('world', default='stage_1.world')
    ...
    ugv_controller = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(pkg_marsupial_simulator_ros2, 'launch', 'ugv_control.launch.py')
            ),
        )
    ...
    spawn_person_node = Node(
            package='gazebo_ros',
            executable='spawn_entity.py',
            arguments=['-entity', 'human_male_1', '-file', os.path.join(get_package_share_directory('marsupial_simulator_ros2'), 'models', 'human_male_1/model.sdf'), 
                       '-x', '-2', '-y', '-2', '-z', '0'],
            output='screen',
        ) 
